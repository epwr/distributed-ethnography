...
def data_service(populated_db_driver) -> DataService:
    return DataService(driver=populated_db_driver)
...
def test_data_service_can_insert_a_survey(
    data_service: DataService, test_survey_open: Survey
):
    data_service.insert_survey(test_survey_open)

    surveys = data_service.get_open_surveys()
    matched_surveys = [
        survey for survey in surveys if survey.uid == test_survey_open.uid
    ]
